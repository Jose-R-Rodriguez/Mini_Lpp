...
id_list(A) ::= TK_ID(B) ids(C).
								{A= new IdListNode(C, {new IdNode(B)});}
ids(A) ::= ids(B) TK_COMMA TK_ID(C).						
								{A= new IdListNode(B, {new IdNode(C)});}
ids(A) ::= .												
								{A= nullptr;}
...
if_statement(A) ::= KW_SI expr(B) opt_eol KW_ENTONCES TK_NEW_LINE statement_list(C) else_statement(D) KW_FIN KW_SI.
								{A= new IfStatementNode(B, C, D);}
else_statement(A) ::= KW_SINO TK_NEW_LINE statement_list(B).
								{A= B;}
else_statement(A) ::= .									
								{A= nullptr;}
for_statement(A) ::= KW_PARA assignment(B) KW_HASTA expr(C) KW_HAGA opt_eol statement_list(D) KW_FIN KW_PARA.
